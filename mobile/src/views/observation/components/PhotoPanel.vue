...
import iconPlus from '@/assets/ods/scr004/icon-plus.svg'
...
type GridSlot =
  | { kind: 'photo'; photo: ObservationPhotoItem; index: number }
  | { kind: 'empty' }

const gridSlots = computed((): GridSlot[] => {
  const slots: GridSlot[] = photos.value.map((photo, index) => ({
    kind: 'photo',
    photo,
    index,
  }))
  while (slots.length < maxCount.value) {
    slots.push({ kind: 'empty' })
  }
  return slots.slice(0, maxCount.value)
})
...
<template>
  <OdsCard v-if="isScr004" class="panel panel--scr004" aria-label="관찰 사진">
    ...
    <p v-if="!isScr004" class="hint">최대 {{ maxCount }}장 · 첫 번째 사진이 대표사진입니다.</p>

    <div v-if="!isScr004" class="actions">
    ...
    <ul v-if="isScr004" class="grid" role="list">
      <li v-for="(slot, slotIndex) in gridSlots" :key="slotIndex" class="grid__cell" role="listitem">
        <button
          v-if="slot.kind === 'photo'"
          type="button"
          class="grid__thumb"
          :aria-label="`${cardLabel(slot.photo, slot.index).fullName} 확대 보기`"
          @click="openViewer(slot.index)"
        >
          <img
            class="grid__img"
            :src="photoThumbSrc(slot.photo)"
            :alt="cardLabel(slot.photo, slot.index).fullName"
            loading="lazy"
          >
          <span v-if="slot.index === 0 && slot.photo.is_representative" class="grid__rep">대표</span>
          <span
            class="grid__del"
            role="button"
            tabindex="0"
            aria-label="사진 삭제"
            @click.stop="removePhoto(slot.photo)"
            @keydown.enter.stop.prevent="removePhoto(slot.photo)"
          >×</span>
        </button>
        <button
          v-else
          ...
          @click="openGallery"
        >
          <img class="grid__plus" :src="iconPlus" alt="" aria-hidden="true">
        </button>
      </li>
    </ul>

    <ul v-else-if="photos.length" class="list">
    ...
    <p v-else-if="!loading && !isScr004" class="empty">등록된 사진이 없습니다. 촬영 또는 갤러리에서 추가하세요.</p>

    <div v-if="isScr004" class="actions actions--scr004">
      <OdsButton variant="primary" :disabled="!canAdd" :block="false" @click="openCamera">
...
.grid__empty {
  border: 2px dashed color-mix(in srgb, var(--ods-color-primary) 45%, var(--ods-color-border));
