...
| Stage7B-2 (private main) | 신규 일반수금 append UI → `SalesPaymentService.add_payment` · `list_payment_methods` SSOT · protected+unpaid add 허용 · edit/delete 계속 disable · **P1 COMMIT/UI 경계** |
...
| Stage7B-2 | **완료 · private main · 운영 미배포** — 신규 append (`add_payment`) · protected 판매도 unpaid>0 이면 add 허용 · P1 COMMIT/UI 경계 |
