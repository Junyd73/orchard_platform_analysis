...
| 18 | 출고확정 CONFIRMED read-only | PC `execute_full_save`가 출고 판매도 재저장 | DEC-031: CONFIRMED + 추적키 → read-only · Stage7A feature |
...
| Stage7A | protected 판매 수금 버튼 disable (full-save 경유 차단). DEC-032 본구현은 Stage7B |
