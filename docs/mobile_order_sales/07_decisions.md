...
| DEC-035 | 수확잔량 SSOT · HARVEST N:M 부분소진 · `t_harvest_consumption` (**OPERATIONAL PASS**) | DEC-035 |
...
| 구현 | **APPROVED · IMPLEMENTED** — Stage7B-1 immutable + Stage7B-2 append · private main · P1 COMMIT/UI 경계 · `slip_no`=내부 회계 linkage |
...
| Stage7B-2 | CONFIRMED unpaid → `open_payment_add_dialog` · stale 차단 · **P1 COMMIT/UI 경계 구현 완료** · private main |
...
| 구현 | **Stage7B-1 · private main** — `assert_sales_total_not_below_paid` · `execute_full_save` DELETE/INSERT 전 |
...
| | |
|--|--|
| 상태 | **APPROVED** (설계 확정) · **IMPLEMENTED** · **REHEARSAL PASS** · **OPS APPLIED** · **OPERATIONAL PASS** — 2026-08-27 설계 · 2026-08-28 구현·rehearsal · 2026-08-31 운영 완료 |
| 결정 | 아래. |
...
| idempotency / cancel-reversal | 멱등 DDL · `is_valid` · guard — **유지** (운영 적용 후에도 동일 · **OPEN** 정책) |

**design/implementation OPEN-DDL → CLOSED.** **PC OPS DDL → APPLIED.** **Lightsail OPS DDL → APPLIED.** **OPS code deploy → APPLIED** (`4daae03`).
...
| `4daae03` | docs / readiness |
| `10632eb` | mobile `build:staging` TS6133 fix |
| `6e9ae87` | dev server API→Vite startup gate |
...
**Stage B (2026-09-02 IMPLEMENTED):** 경락후보 조회만. 특정일 `trade_dt` · 정산 API 원본 우선 · 유효 0건 시 realtime · local settlement table 미사용 · farm 시/군·법인·품종·kg 자동필터 · 최종 선택은 사용자 · realtime 등급 입력 필요 · N행 보존 · 저장/재고 변경 없음. **SETTLEMENT `unit_tot_qty`(총 kg) → candidate `qty`(박스수=`raw_qty/spec_kg`, 정수만); REALTIME qty 변환 없음; `source_key`는 raw kg identity 유지.** **SETTLEMENT box_qty는 total weight / unit weight가 SSOT이며, total amount / average price는 박스수 validation hard-gate로 사용하지 않는다.** **배 전체 canonical `fruit_count_bucket`(과수 구간 키): `weight + FR020101~107` explicit 표(15/7.5/5kg만) · 외부 size label은 승인 bucket만 인정 · raw `size_name`/`source_key` 보존 · Core SSOT · DDL 없음 · candidate/detail API에 `fruit_count_bucket` 노출.**

**Stage C (2026-09-02 IMPLEMENTED, Core only):** `AuctionFinalizeService`. 서버 candidate 재조회 후 selected `source_key` snapshot. 클라이언트 qty/price/amount 미신뢰. stale key reject. `(source_key) WHERE is_valid=1` 전역 활성 UNIQUE. 같은 외부 경락 원본행은 농장이 달라도 두 shipment에 매칭 불가. 내부 spec(`spec_*_cd`)과 외부 `source_grade_*` 분리. 비교단위 = shipment spec (variety+grade+size+weight), 1 spec ↔ N match. SETTLEMENT는 등급 **이름** normalize + **`fruit_count_bucket`**(weight+FR0201xx / 외부 size label) 매칭(raw size_name exact 불필요). REALTIME은 사용자 `grade_cd` + size bucket. `diff_qty = matched_qty − farm_shipped_qty`. diff=0이면 discrepancy 없음(보내면 reject). diff≠0이면 spec당 활성 discrepancy 1건 필수 (`QTY_ERROR`/`RETURN`/`DAMAGE`/`OTHER`). RETURN만 `in_qty` 역FIFO IN + `AUCTION_SHIP` IN log (`경매반품`). 판매 `sales_source=AUCTION` · `sales_dt=trade_dt` · 1 match = 1 sales_detail · `stock_seq=NULL` · gross(수수료/운송 차감 0) · 추가 OUT/SALE log 0 · shipment `COMPLETED` + `sales_no`/`match_trade_dt`. 중복 finalize 거부. 정정은 Stage F.
