...
> **현재 기준 (2026-08-23):** private main **Stage6-0 반영** · 운영 backend **`b48ca8b`**(Stage5).  
...
> **Stage6A:** 판매상세 GET + Mobile read-only — **완료 · feature branch · main 미반영**.  
> **다음:** Stage6 **6B 수금내역**.  
...
| — | Stage6A 판매상세 read-only | **완료 · feature · main 미반영** |
| — | **다음:** Stage6 **6B 수금내역** | [§ 2026-08-21](#2026-08-21--선입금수금-정책-확정) |
...
SHA 스냅샷: Stage6-0 merge = `5782681` · feature `cb6e28c` · review `44da339` · 운영 backend = `b48ca8b`(Stage5) · Stage6-0 = **완료 · private main · 운영 미배포**.
...
6A 판매상세 read-only  ← 완료 · feature · main 미반영
6B 수금내역 · 6C 수금등록
7  PC 정합성
...
| 6A | **판매상세 read-only** | **완료 · feature · main 미반영** | `GET /sales/{sales_no}` · `SalesDetailView` · cash SUM SSOT · FIFO UI grouping · 배송 제외 · SELECT only | 6-0 |
| 6 | **수금내역/수금등록** | **예정** (6B→6C) | 6B 수금내역 → 6C 수금등록(수금액 ≤ 미수금, 결제수단 필수) | 6A |
