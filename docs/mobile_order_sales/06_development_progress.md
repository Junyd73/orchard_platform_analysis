...
> **Stage4 (출고 선입금 자동배분):** feature 구현 완료 · **main/운영 미반영**.  
...
2 선입금 결제수단 기반  ← 완료 · 운영
3 판매 수금 Core        ← 완료 · 운영
4 출고 시 선입금 자동배분  ← feature 구현 완료 · main/운영 미반영
→ 5 판매목록  →  6 판매상세/수금등록
...
| 4 | **출고 시 선입금 자동배분** | **feature 구현 완료 · main/운영 미반영** | `OrderShipService.confirm()` 동일 TX · `PaymentAddIn.source_order_no` · provenance=`t_cash_ledger.order_no` · 순차 배분 (DEC-019) · DDL/HTTP/Mobile/PC 0 | — |
