...
> **현재 기준 (2026-08-21):** private main **`afce894`**. 운영 backend **`a41b40e`**. analysis main **`4eaf621`**.  
> Stage 6 출고/판매 · Order→Ship Step1~3 · 주문목록 compact · **2단계 DEC-028 완료·운영**.  
> **3단계 판매 수금 Core:** feature `cursor/sales-payment-stage3` 구현 완료 · **main/운영 미반영**.  
...
| — | **다음:** 3단계 main 반영 → 4 출고 선입금 배분 → 판매목록/상세 → PC 정합 → 가락 | [§ 2026-08-21](#2026-08-21--선입금수금-정책-확정) |
...
1 설계서 정합성 완료
2 선입금 결제수단 기반  ← 완료 · 운영
3 판매 수금 Core        ← feature Core 완료 / main·운영 미반영
→ 4 출고 시 선입금 자동배분  →  5 판매목록  →  6 판매상세/수금등록
→ 7 PC 정합성  →  8 가락 DRAFT→CONFIRMED
```

SHA 스냅샷: private main = `afce894` · 운영 backend = `a41b40e` · analysis main = `4eaf621`.
...
| 3 | **판매 수금 Core** | **feature 구현 완료 / main·운영 미반영** | `SalesPaymentService` append 추가수금. cash SSOT · AccountManager SALE · DRAFT 금지 · `AS0101` 검증 · `add_payment_in_tx` caller-owned TX. HTTP/UI 없음 | main merge · 운영 배포 승인 |
