...
### 7.2 GET 상세 (Stage6A · read-only · main 미반영)
...
> **HTTP GET/PUT:** payments GET/PUT **미구현** (개발순서 6B·6C). **판매상세 GET는 Stage6A read-only 구현** (main 미반영).
...
**Stage6A (feature · main 미반영):** `GET /farms/{farm_cd}/sales/{sales_no}` · `SalesQueryService.get_sale_detail` · Mobile `SalesDetailView` (`/orders/sales/:salesNo`).
