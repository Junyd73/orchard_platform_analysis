...
**PC 수금 append-only (DEC-032 · Stage7B-1 일부):** 기존 수금 수정/삭제 금지 · `execute_full_save`에서 cash/ledger mutation **제거** · master `tot_paid_amt`/`tot_unpaid_amt`는 `SalesPaymentService.get_payment_summary` cash SSOT · 수금 UI read-only · add/edit/delete disable (Stage7B-2에서 신규 append).

**신규 판매 수금 (DEC-033 · APPROVED):** 판매 저장 전 수금등록 금지 · CONFIRMED 저장 후 별도 수금등록(Stage7B-2).

**판매금액 감액 (DEC-034 · APPROVED):** 일반 직접판매 수정 시 `new tot_sales_amt >= actual paid(cash SSOT)` — 미만이면 저장 전체 차단.
