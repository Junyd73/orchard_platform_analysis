...
from ui.pages.labor_cost_detail_page import LaborCostDetailPage
...
class DashboardPage(QWidget):
    ...
    def init_ui(self):
        ...
        alert_title = QLabel("알림", styleSheet=MainStyles.CARD_LBL_STYLE)
        alert_title.setFont(make_app_font(12, bold=True))
        al.addWidget(alert_title)
        self.alert_label = QLabel("오늘의 알림이 없습니다.")
        self.alert_label.setStyleSheet("color: #718096;")
        self.alert_label.setFont(make_app_font(11))
    ...
    def _create_briefing_bar(self):
        ...
        for w in [
            ...
            w.setStyleSheet(st)
            w.setFont(make_app_font(12, bold=True))
        for w in [
            self.lbl_date,
            self.lbl_farm,
        ...
        self.btn_edit_mode.setFont(make_app_font(12))
        ...
        self.btn_card_manage.setFont(make_app_font(12))
        self.btn_card_manage.setFixedHeight(28)
    ...
    def _open_card_manage_dialog(self):
        ...
        root.setSpacing(10)
        hidden_desc = QLabel("숨긴 카드를 다시 표시할 수 있습니다.", styleSheet="color:#718096;")
        hidden_desc.setFont(make_app_font(12))
        root.addWidget(hidden_desc)
        for cid in hidden:
            title = CARDS_META[cid][0]
            icon = CARDS_META[cid][1]
            row = QHBoxLayout()
            title_lbl = QLabel(f"{icon}  {title}", styleSheet="color:#2D3748;")
            title_lbl.setFont(make_app_font(13))
    ...
    def _show_detail_page(self, page_id):
        ...
        if normalized_page_id not in self._detail_pages:
            if normalized_page_id == "market":
                ...
                self._detail_pages[normalized_page_id] = LaborCostDetailPage(self.db, self.session, self)
    ...
    def load_summary_data(self):
        ...
        self.lbl_cost.setText("오늘 비용 0원")
