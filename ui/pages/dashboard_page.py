...
class WeatherWorker(QObject):
    """메인 스레드에서 WeatherManager/API 호출 금지 — 워커 전용 DB 연결 사용."""

    finished = pyqtSignal(dict)

    def __init__(self, db_file: str, farm_cd: str, target_date: str):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            data = wm.get_dashboard_weather(self._farm_cd, self._target_date) or {}
...
class DashboardPage(QWidget):
    ...
    def __init__(self, db_manager, session):
        ...
        self._weather_seq = 0
        self._weather_reload_pending = False
    ...
    def load_summary_data(self):
        ...
        self.lbl_weather.setText("날씨 불러오는 중...")
        if "weather" in self._cards and hasattr(self._cards["weather"], "set_weather_summary_data"):
            self._cards["weather"].set_weather_summary_data(
                {
                    "current_status": "불러오는 중...",
                    "current_temp": "-",
                    "today_max": None,
                    "today_min": None,
                    "summary": "",
        ...
        QTimer.singleShot(0, self.load_weather_async)

    def load_weather_async(self):
        if self._weather_thread is not None and self._weather_thread.isRunning():
            self._weather_reload_pending = True
            return
        self._weather_reload_pending = False
        self._weather_seq += 1
        ...
        thread = QThread()
        worker = WeatherWorker(db_file, self.farm_cd, today)
        ...
        worker.finished.connect(partial(self._on_weather_loaded, seq, thread, worker))

        thread.start()

    def _on_weather_loaded(self, seq: int, thread: QThread, worker: QObject, raw: dict):
        try:
            if seq != self._weather_seq:
                return
            payload = self._weather_raw_to_card_payload(raw or {})
            self.lbl_weather.setText(
                f"날씨 {payload.get('current_status', '—')} {payload.get('current_temp', '—')}"
            )
            if "weather" in self._cards and hasattr(
                self._cards["weather"], "set_weather_summary_data"
            ):
                self._cards["weather"].set_weather_summary_data(payload)
        finally:
            thread.quit()
            thread.wait()
            worker.deleteLater()
            ...
            if self._weather_thread is thread:
                self._weather_thread = None
                self._weather_worker = None
            if self._weather_reload_pending:
                self._weather_reload_pending = False
                QTimer.singleShot(0, self.load_weather_async)

    def _weather_raw_to_card_payload(self, raw: dict):
        fallback = {
            "current_temp": "—",
        ...
        try:
            ...
            current_temp = "—" if temp_now is None else f"{temp_now}°"
            return {
                "current_temp": current_temp,
                "current_status": str(raw.get("weather_text") or "데이터 없음"),
                "today_max": raw.get("temp_max"),
                "today_min": raw.get("temp_min"),
                "summary": "",
                "daily_ranges": list(raw.get("weekly") or []),
            }
        except Exception as e:
            print(f"[Dashboard] weather card payload failed: {e}")
            return fallback
