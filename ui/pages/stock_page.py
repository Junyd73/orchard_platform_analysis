...
class InventoryCard(QFrame):
    ...
    def __init__(self, data):
        ...
        layout.addWidget(QLabel(f"📅 {data['storage_dt']}", styleSheet="font-size: 8pt; color: #718096; border: none;"))
        layout.addWidget(QLabel(f"<b>{data['size_nm']}</b>", styleSheet="font-size: 10pt; color: #2D3748; border: none;"))
        layout.addStretch()
        self.qty_lbl = QLabel(f"{self.current_qty}개"); self.qty_lbl.setStyleSheet("font-size: 11pt; color: #38A169; font-weight: bold; border: none;")
...
class ProductCard(QFrame):
    ...
    def __init__(self, data):
        ...
        header.setStyleSheet("font-size: 8.5pt; border: none;"); layout.addWidget(header)
        ...
        self.qty_lbl.setStyleSheet("font-size: 11pt; color: #DD6B20; font-weight: bold; border: none;")
        layout.addWidget(self.qty_lbl, alignment=Qt.AlignmentFlag.AlignRight)
...
class WorkCartCard(QFrame):
    ...
    def __init__(self, data, max_stock=9999):
        ...
        header.addWidget(QLabel(f"<b>{title}</b>", styleSheet="font-size: 9pt; border: none;"))
        ...
        self.spin = QSpinBox(); self.spin.setRange(0, self.max_allowed); self.spin.setSuffix(" C" if not is_p else " B"); self.spin.setFixedHeight(26); self.spin.setStyleSheet("font-size: 10pt; border: 1px solid #CBD5E0;"); self.spin.valueChanged.connect(self.sync_qty); layout.addWidget(self.spin)
...
class GradeInputTile(QFrame):
    ...
    def __init__(self, grade_code, grade_nm, initial_val=0):
        ...
        layout.addWidget(QLabel(f"<b>{self.grade_nm}</b>", styleSheet="font-size: 11pt; color: #2D3748; border: none;"))
        self.spin = QSpinBox()
        self.spin.setRange(0, 999); self.spin.setValue(initial_val); self.spin.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.spin.setStyleSheet("font-size: 13pt; font-weight: bold; color: #3182CE; border: 1px solid #CBD5E0; height: 30px;")
        ...
        for v in [1, 10]:
            b = QPushButton(f"+{v}"); b.setFixedHeight(26); b.setStyleSheet(MainStyles.BTN_PRIMARY + "font-size: 9pt; font-weight: bold;")
...
class StockPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.yield_bar.setStyleSheet("QProgressBar { border: 1px solid #CBD5E0; border-radius: 4px; text-align: center; font-size: 8pt; font-weight: bold; color: #2D3748; } QProgressBar::chunk { background-color: #48BB78; }")
    ...
    def save_production_log(self):
        ...
        try:
            ...
            ref = self.selected_cards[0].data  # 기준 정보 (work_id 포함)
            queries = []

            # ==========================================================
            # 1. [작업 상태 변경] 작업 지시를 완료(DONE)로 업데이트
            # ==========================================================
            # 이 로직이 빠지면 작업 목록에서 해당 지시가 사라지지 않습니다.
            sql_work_done = """
                UPDATE t_work_detail 
                SET status_cd = 'DONE', mod_id = ?, mod_dt = datetime('now','localtime')
                WHERE work_id = ?
            """
            queries.append((sql_work_done, (self.user_id, ref['work_id'])))
