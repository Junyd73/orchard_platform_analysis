...
from PyQt6.QtCore import QDate, Qt, QTimer, QTime, QEvent, QThread, QObject
from PyQt6.QtWidgets import *
from PyQt6.QtGui import *
from ui.styles import MainStyles
from ui.widgets.work_log.work_log_month_calendar import WorkLogMonthCalendar
from ui.widgets.work_log.weather_fetch_worker import WeatherFetchWorker
...
# QStackedWidget 페이지 인덱스
WORK_LOG_PAGE_MONTHLY = 0
WORK_LOG_PAGE_DAILY = 1
WORK_LOG_ENTRY_MONTHLY = "monthly"
WORK_LOG_ENTRY_DAILY = "daily"
# 날씨 가져오기 버튼 문구
WORK_LOG_BTN_WEATHER_FETCH = "날씨 가져오기"
WORK_LOG_BTN_WEATHER_BUSY = "날씨 조회 중..."
...
class WorkLogPage(QWidget):
    def __init__(self, db_manager, session, entry_mode=WORK_LOG_ENTRY_DAILY):
        super().__init__()
        self.db = db_manager
        ...
        mode = (entry_mode or WORK_LOG_ENTRY_DAILY).strip().lower()
        self.entry_mode = (
            WORK_LOG_ENTRY_MONTHLY
            if mode == WORK_LOG_ENTRY_MONTHLY
            else WORK_LOG_ENTRY_DAILY
        )
        ...
        self._monthly_cache_key = None
        self._monthly_overview = None
        self._monthly_load_seq = 0
        self._monthly_view_year = QDate.currentDate().year()
        self._monthly_view_month = QDate.currentDate().month()
        self._weather_fetch_seq = 0
        self._weather_thread = None
        self._weather_worker = None
        self._weather_last_elapsed = 0.0
        self._weather_fetch_busy = False
        self._weather_page_closing = False

        self.init_ui()
        self.load_initial_codes()
        self._update_pesticide_tab_state()
        if self.entry_mode == WORK_LOG_ENTRY_MONTHLY:
            QTimer.singleShot(200, self.refresh_monthly_overview)
        else:
            QTimer.singleShot(
                200, lambda: self.open_daily_for_date(QDate.currentDate())
            )
    ...
    def _safe_show_status(self, message: str) -> None:
        """MainApp.show_status가 없어도 예외가 나지 않도록 안전 호출."""
        try:
            win = self.window()
            fn = getattr(win, "show_status", None)
            if callable(fn):
                fn(message)
        except Exception:
            pass
    ...
    def init_ui(self):
        """월간정보 / 일간정보 QStackedWidget 구성. 기존 상세 UI는 daily_page에 유지."""
        self.setStyleSheet(MainStyles.MAIN_BG)
        ...
        self._init_monthly_page(self.monthly_page)
        self.stack.addWidget(self.monthly_page)

        self.daily_page = QWidget()
        self._init_daily_page(self.daily_page)
        self.stack.addWidget(self.daily_page)

        if self.entry_mode == WORK_LOG_ENTRY_MONTHLY:
            self.stack.setCurrentIndex(WORK_LOG_PAGE_MONTHLY)
        else:
            self.stack.setCurrentIndex(WORK_LOG_PAGE_DAILY)

    def _init_monthly_page(self, page: QWidget) -> None:
        """1페이지: 월간정보 (큰 캘린더 + 요약 카드)."""
        page.setStyleSheet(MainStyles.MAIN_BG)
        lay = QVBoxLayout(page)
        lay.setContentsMargins(15, 12, 15, 15)
        lay.setSpacing(12)

        # 상단: 제목 | (중앙 연월) | 이전 / 오늘 / 다음 / 작성
        top = QHBoxLayout()
        top.setSpacing(8)
        self.lbl_monthly_title = QLabel("영농일지 월간정보")
        self.lbl_monthly_title.setStyleSheet(MainStyles.WORK_LOG_PAGE_TITLE)
        top.addWidget(self.lbl_monthly_title, 0, Qt.AlignmentFlag.AlignVCenter)
        top.addStretch(1)

        self.lbl_month_ym = QLabel("")
        self.lbl_month_ym.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_YM_TITLE)
        self.lbl_month_ym.setAlignment(Qt.AlignmentFlag.AlignCenter)
        top.addWidget(self.lbl_month_ym, 0, Qt.AlignmentFlag.AlignVCenter)
        top.addStretch(1)

        self.btn_month_prev = QPushButton("이전 달")
        self.btn_month_prev.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_NAV_BTN)
        self.btn_month_prev.setFixedHeight(36)
        self.btn_month_prev.clicked.connect(self._on_month_prev)

        self.btn_month_today = QPushButton("오늘")
        self.btn_month_today.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_NAV_BTN)
        self.btn_month_today.setFixedHeight(36)
        self.btn_month_today.clicked.connect(self._on_month_today)

        self.btn_month_next = QPushButton("다음 달")
        self.btn_month_next.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_NAV_BTN)
        self.btn_month_next.setFixedHeight(36)
        self.btn_month_next.clicked.connect(self._on_month_next)

        self.btn_open_daily_today = QPushButton("영농일지 작성")
        self.btn_open_daily_today.setStyleSheet(MainStyles.BTN_PRIMARY)
        self.btn_open_daily_today.setFixedHeight(36)
        self.btn_open_daily_today.clicked.connect(self._on_write_today_daily)

        for w in (
            self.btn_month_prev,
            self.btn_month_today,
            self.btn_month_next,
            self.btn_open_daily_today,
        ):
            top.addWidget(w, 0, Qt.AlignmentFlag.AlignVCenter)
        lay.addLayout(top)

        summary_row = QHBoxLayout()
        summary_row.setSpacing(10)
        self._monthly_summary_labels = {}
        summary_defs = (
            ("work_day_count", "작업일 수"),
            ("work_count", "전체 작업 건수"),
            ("resource_count", "인력 투입 건수"),
            ("labor_sum", "인건비 합계"),
            ("expense_sum", "경비·자재 합계"),
        )
        for key, title in summary_defs:
            card = QFrame()
            card.setObjectName("WorkLogMonthlySummaryCard")
            card.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_SUMMARY_CARD)
            ...
            c_lay.setSpacing(6)
            t = QLabel(title)
            t.setObjectName("WorkLogMonthlySummaryTitle")
            v = QLabel("0")
            v.setObjectName("WorkLogMonthlySummaryValue")
            ...
            self._monthly_summary_labels[key] = v
        lay.addLayout(summary_row)

        cal_card = QFrame()
        cal_card.setObjectName("WorkLogMonthlyCalCard")
        cal_card.setStyleSheet(MainStyles.WORK_LOG_MONTHLY_CAL_CARD)
        cal_card.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        cal_lay = QVBoxLayout(cal_card)
        cal_lay.setContentsMargins(10, 10, 10, 10)
        cal_lay.setSpacing(0)

        self.month_calendar = WorkLogMonthCalendar()
        self.month_calendar.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        self.month_calendar.dayActivated.connect(self._on_monthly_day_activated)
        self.month_calendar.futureDateBlocked.connect(self._on_future_date_blocked)
        cal_lay.addWidget(self.month_calendar, 1)
        lay.addWidget(cal_card, 1)
        self._sync_monthly_header_labels()

    def _init_daily_page(self, page: QWidget) -> None:
        """2페이지: 기존 영농일지 상세 UI (기능 로직 재사용)."""
        main_layout = QVBoxLayout(page)
        main_layout.setContentsMargins(0, 0, 0, 0)
        main_layout.setSpacing(0)

        # --- 상단 고정 제어 바 (스크롤 되지 않음) ---
        top_bar = QFrame()
        ...
        self.btn_back_monthly = QPushButton("← 월간정보")
        self.btn_back_monthly.setStyleSheet(MainStyles.BTN_SECONDARY)
        self.btn_back_monthly.setFixedHeight(36)
        self.btn_back_monthly.clicked.connect(self.show_monthly_page)
        top_layout.addWidget(self.btn_back_monthly)

        self.lbl_daily_title = QLabel("영농일지")
        self.lbl_daily_title.setStyleSheet(MainStyles.WORK_LOG_PAGE_TITLE)
        top_layout.addWidget(self.lbl_daily_title)

        self.btn_obs_add = QPushButton("관찰 등록")
        self.btn_obs_add.setStyleSheet(MainStyles.BTN_SECONDARY_COMPACT)
        self.btn_obs_add.setFixedHeight(36)
        self.btn_obs_add.clicked.connect(self._open_observation_new)
        top_layout.addWidget(self.btn_obs_add)
        self.btn_obs_list = QPushButton("이날 관찰 조회 (0건)")
        self.btn_obs_list.setStyleSheet(MainStyles.BTN_SECONDARY_COMPACT)
        self.btn_obs_list.setFixedHeight(36)
        self.btn_obs_list.clicked.connect(self._open_observation_list_today)
        top_layout.addWidget(self.btn_obs_list)
        ...
        self.date_edit.dateChanged.connect(self._update_daily_title)
        self.date_edit.dateChanged.connect(self._on_weather_context_changed)
        w_head.addWidget(self.date_edit)
        self._update_daily_title() 
        w_head.addStretch()
        ...
        self.btn_w_fetch = QPushButton(WORK_LOG_BTN_WEATHER_FETCH)
        self.btn_w_fetch.setStyleSheet(MainStyles.BTN_FETCH)
        self.btn_w_fetch.clicked.connect(self.fetch_weather_api)
        w_head.addWidget(btn_w_load); w_head.addWidget(self.btn_w_fetch)
    ...
    # 월간정보 ↔ 일간정보 전환 / 월간 집계
    # ------------------------------------------------------------------
    def _sync_monthly_header_labels(self) -> None:
        y = int(self._monthly_view_year)
        m = int(self._monthly_view_month)
        if hasattr(self, "lbl_month_ym") and self.lbl_month_ym is not None:
            self.lbl_month_ym.setText(f"{y}년 {m}월")
        self._update_month_nav_enabled()

    def _update_month_nav_enabled(self) -> None:
        """현재 월이면 '다음 달' 비활성화 (미래 월 이동 차단)."""
        if not hasattr(self, "btn_month_next") or self.btn_month_next is None:
            return
        today = QDate.currentDate()
        at_current_or_future = (
            int(self._monthly_view_year) > today.year()
            or (
                int(self._monthly_view_year) == today.year()
                and int(self._monthly_view_month) >= today.month()
            )
        )
        self.btn_month_next.setEnabled(not at_current_or_future)

    def _update_daily_title(self, *_args) -> None:
        if not hasattr(self, "lbl_daily_title") or self.lbl_daily_title is None:
            return
        if not hasattr(self, "date_edit") or self.date_edit is None:
            return
        d = self.date_edit.date()
        self.lbl_daily_title.setText(
            f"{d.year()}년 {d.month()}월 {d.day()}일 영농일지"
        )

    def invalidate_monthly_cache(self) -> None:
        self._monthly_cache_key = None

    def refresh_monthly_overview(self, force: bool = False) -> None:
        """선택 월 데이터만 재조회(빠른 월 전환 시 오래된 응답은 무시)."""
        y = int(self._monthly_view_year)
        m = int(self._monthly_view_month)
        cache_key = (str(self.farm_cd or ""), y, m)
        if (
            not force
            and self._monthly_cache_key == cache_key
            and isinstance(self._monthly_overview, dict)
        ):
            self._apply_monthly_overview(self._monthly_overview)
            return

        self._monthly_load_seq += 1
        seq = self._monthly_load_seq
        self._sync_monthly_header_labels()
        try:
            overview = self.db.get_work_log_monthly_overview(self.farm_cd, y, m)
        except Exception as e:
            print(f"[WorkLog] monthly overview error: {e}")
            overview = {
                "year": y,
                "month": m,
                "summary": {
        ...
        if seq != self._monthly_load_seq:
            return
        self._monthly_cache_key = cache_key
        self._monthly_overview = overview or {}
        self._apply_monthly_overview(self._monthly_overview)

    def _apply_monthly_overview(self, overview: dict) -> None:
        summary = (overview or {}).get("summary") or {}
        days = (overview or {}).get("days") or {}
        ...
        labels = getattr(self, "_monthly_summary_labels", {}) or {}
        if "work_day_count" in labels:
            labels["work_day_count"].setText(_fmt_count(summary.get("work_day_count")))
        if "work_count" in labels:
            labels["work_count"].setText(_fmt_count(summary.get("work_count")))
        if "resource_count" in labels:
            labels["resource_count"].setText(_fmt_count(summary.get("resource_count")))
        if "labor_sum" in labels:
            labels["labor_sum"].setText(_fmt_amt(summary.get("labor_sum")))
        if "expense_sum" in labels:
            labels["expense_sum"].setText(_fmt_amt(summary.get("expense_sum")))

        if hasattr(self, "month_calendar") and self.month_calendar is not None:
            self.month_calendar.refresh_max_date()
            self.month_calendar.setCurrentPage(
                int(self._monthly_view_year), int(self._monthly_view_month)
            )
            self.month_calendar.set_day_data(days)
            sel = self.month_calendar.selected_work_date()
            if (
                sel.isValid()
                and sel.year() == int(self._monthly_view_year)
                and sel.month() == int(self._monthly_view_month)
                and sel <= QDate.currentDate()
            ):
                self.month_calendar.set_selected_work_date(sel)
        self._update_month_nav_enabled()

    def _shift_month(self, delta: int) -> None:
        d = QDate(int(self._monthly_view_year), int(self._monthly_view_month), 1)
        d = d.addMonths(int(delta))
        today = QDate.currentDate()
        # 미래 월로는 이동하지 않음
        if d.year() > today.year() or (
            d.year() == today.year() and d.month() > today.month()
        ):
            d = QDate(today.year(), today.month(), 1)
        self._monthly_view_year = d.year()
        self._monthly_view_month = d.month()
        self._sync_monthly_header_labels()
        self.refresh_monthly_overview(force=True)

    def _on_month_prev(self) -> None:
        self._shift_month(-1)

    def _on_month_next(self) -> None:
        self._shift_month(1)

    def _on_month_today(self) -> None:
        today = QDate.currentDate()
        self._monthly_view_year = today.year()
        self._monthly_view_month = today.month()
        if hasattr(self, "month_calendar") and self.month_calendar is not None:
            self.month_calendar.refresh_max_date()
            self.month_calendar.set_selected_work_date(today)
        self._sync_monthly_header_labels()
        self.refresh_monthly_overview(force=True)

    def _on_write_today_daily(self) -> None:
        self.open_daily_for_date(QDate.currentDate())

    def _on_future_date_blocked(self, message: str) -> None:
        self._safe_show_status(
            message or "미래 날짜의 영농일지는 작성할 수 없습니다."
        )

    def _on_monthly_day_activated(self, qdate: QDate) -> None:
        if not qdate or not qdate.isValid():
            return
        if qdate > QDate.currentDate():
            self._on_future_date_blocked(
                "미래 날짜의 영농일지는 작성할 수 없습니다."
            )
            return
        self.open_daily_for_date(qdate)

    def _obs_nav_host(self):
        win = self.window()
        return win if win is not None and callable(getattr(win, "open_menu", None)) else None

    def _refresh_observation_day_count(self) -> None:
        if not hasattr(self, "btn_obs_list"):
            return
        farm = str(getattr(self, "farm_cd", "") or "").strip()
        if not farm or not hasattr(self, "date_edit"):
            self.btn_obs_list.setText("이날 관찰 조회 (0건)")
            return
        dt = self.date_edit.date().toString("yyyy-MM-dd")
        try:
            n = int(self.db.count_observations_on_date(farm, dt) or 0)
        except Exception:
            n = 0
        self.btn_obs_list.setText(f"이날 관찰 조회 ({n}건)")

    def _open_observation_new(self) -> None:
        if not hasattr(self, "date_edit"):
            return
        if not self._ensure_work_date_not_future():
            return
        host = self._obs_nav_host()
        if host is None:
            QMessageBox.information(
                self, "관찰", "관찰일지 메뉴로 이동할 수 없습니다."
            )
            return
        from core.db_manager import DBManager

        obs_dt = self.date_edit.date().toString("yyyy-MM-dd")

        def _after(page):
            fn = getattr(page, "open_new_observation", None)
            if callable(fn):
                fn(obs_dt=obs_dt)
            self._refresh_observation_day_count()

        host.open_menu(DBManager.OBSERVATION_LOG_MENU_CD, after=_after)

    def _open_observation_list_today(self) -> None:
        if not hasattr(self, "date_edit"):
            return
        host = self._obs_nav_host()
        if host is None:
        ...
        def _after(page):
            fn = getattr(page, "apply_external_filters", None)
            if callable(fn):
                fn(date_from=obs_dt, date_to=obs_dt)

        host.open_menu(DBManager.OBSERVATION_LOG_MENU_CD, after=_after)

    def open_daily_for_date(self, qdate: QDate) -> None:
        """날짜를 date_edit에 반영 후 일간정보로 전환·조회. 미래일은 이동하지 않음."""
        if not qdate or not qdate.isValid():
            return
        today = QDate.currentDate()
        target = QDate(qdate)
        if target > today:
            ...
            return
        self._monthly_view_year = target.year()
        self._monthly_view_month = target.month()
        if hasattr(self, "month_calendar") and self.month_calendar is not None:
            self.month_calendar.refresh_max_date()
            self.month_calendar.set_selected_work_date(target)

        if hasattr(self, "date_edit") and self.date_edit is not None:
        ...
        self.update_day_of_week(target)

    def show_monthly_page(self) -> None:
        """직전에 보던 연월·선택일을 유지한 채 월간정보로 복귀."""
        if hasattr(self, "date_edit") and self.date_edit is not None:
            d = self.date_edit.date()
            if d.isValid():
                self._monthly_view_year = d.year()
                self._monthly_view_month = d.month()
                if hasattr(self, "month_calendar") and self.month_calendar is not None:
                    self.month_calendar.set_selected_work_date(d)
        self.stack.setCurrentIndex(WORK_LOG_PAGE_MONTHLY)
        self.refresh_monthly_overview(force=False)
    ...
    def load_master_data(self):
        ...
        try:
            ...
            if work_res:
                ...
                self._safe_show_status(f"✅ [{work_dt}] 정보 조회 완료 (작업 {len(work_res)}건)")
                self._bootstrap_pesticide_cache_from_db()
            else:
                self._safe_show_status(f"ℹ️ [{work_dt}] 저장된 정보가 없습니다.")
    ...
    def display_weather(self, weather_res):
        ...
        if weather_res:
            ...
            self._safe_show_status(f"🌤️ [{row.get('work_dt')}] 날씨와 이슈를 불러왔습니다.")
        else:
            # 데이터가 없는 날짜 초기화
    ...
    # 기능 구현부
    # =========================================================
    # 날씨 가져오기 (비동기 — UI 스레드에서 API/캐시 조회 금지)
    def fetch_weather_api(self):
        """위치 확인·버튼 상태·워커 시작만 메인에서 수행. 캐시/API는 워커 담당."""
        if self._weather_fetch_busy:
            return
        if not self._ensure_work_date_not_future():
            return
        ...
        if not loc_res or not all([loc_res[0][0], loc_res[0][1], loc_res[0][2], loc_res[0][3]]):
            ...
            self._safe_show_status("위치정보 누락으로 날씨를 조회할 수 없습니다.")
            return

        db_path = self._db_file_for_worker()
        if not db_path:
            QMessageBox.warning(
                self,
                "날씨 조회",
            ...
            self._safe_show_status("데이터베이스 파일을 찾을 수 없습니다.")
            return

        lat, lon, nx, ny = loc_res[0]
        work_dt = self.date_edit.date().toString("yyyy-MM-dd")
        self._weather_fetch_seq += 1
        seq = self._weather_fetch_seq
        self._set_weather_fetch_busy(True)
        self._safe_show_status("선택 날짜의 날씨를 조회하고 있습니다.")

        self._start_weather_fetch_worker(
            seq=seq,
            work_dt=work_dt,
            lat=float(lat),
            lon=float(lon),
    ...
    def _on_weather_context_changed(self, *_args) -> None:
        """날짜 변경 시 진행 중 날씨 조회 결과 적용을 무효화한다."""
        self._weather_fetch_seq += 1

    def _set_weather_fetch_busy(self, busy: bool) -> None:
        self._weather_fetch_busy = bool(busy)
        btn = getattr(self, "btn_w_fetch", None)
        if btn is None:
            return
        btn.setEnabled(not busy)
        btn.setText(WORK_LOG_BTN_WEATHER_BUSY if busy else WORK_LOG_BTN_WEATHER_FETCH)

    def _db_file_for_worker(self) -> str | None:
        """메인 DB와 동일 파일의 정규화된 절대경로. 없거나 파일이 없으면 None."""
        db_path = getattr(self.db, "db_name", None)
        if not db_path:
            return None
        abs_path = os.path.realpath(os.path.abspath(str(db_path)))
        if not os.path.isfile(abs_path):
            return None
        return abs_path

    def _start_weather_fetch_worker(
        self,
        *,
        seq: int,
        work_dt: str,
        ...
        if self._weather_thread is not None and self._weather_thread.isRunning():
            # 강제 terminate 금지. 중복 워커 생성 차단.
            self._set_weather_fetch_busy(False)
            self._safe_show_status(
                "이전 날씨 조회가 진행 중입니다. 잠시 후 다시 시도해 주세요."
            )
            return
        ...
        # parent=None: 페이지 종료 시 QThread destroyed while running 방지
        thread = QThread()
        worker = WeatherFetchWorker(
            db_path,
            self.current_farm_cd,
            work_dt,
            lat,
            lon,
            nx,
        ...
        worker.elapsed.connect(partial(self._on_weather_fetch_elapsed, seq))
        worker.finished.connect(partial(self._on_weather_fetch_finished, seq))
        worker.failed.connect(partial(self._on_weather_fetch_failed, seq))
        # 비차단 정리: wait() 금지
        ...
        thread.finished.connect(
            partial(self._on_weather_thread_finished, thread, worker)
        )
        thread.start()

    def _on_weather_thread_finished(self, thread: QThread, worker: QObject) -> None:
        """스레드 종료 후 참조만 정리(이미 deleteLater 연결됨)."""
        if self._weather_thread is thread:
            self._weather_thread = None
        if self._weather_worker is worker:
            self._weather_worker = None

    def _on_weather_fetch_elapsed(self, seq: int, sec: float) -> None:
        if seq == self._weather_fetch_seq:
            self._weather_last_elapsed = float(sec or 0.0)

    def _weather_ui_accepts_result(self, seq: int) -> bool:
        """페이지 종료·날짜 변경·seq 불일치 시 결과 적용 금지."""
        if getattr(self, "_weather_page_closing", False):
            return False
        if seq != self._weather_fetch_seq:
            return False
        try:
    ...
    def _on_weather_fetch_finished(self, seq: int, payload: dict) -> None:
        try:
            if not self._weather_ui_accepts_result(seq):
                return
            data = (payload or {}).get("data")
            req_dt = str((payload or {}).get("work_dt") or "")
            cur_dt = self.date_edit.date().toString("yyyy-MM-dd")
            if req_dt and req_dt != cur_dt:
                return
            if not data:
                return
            self._apply_fetched_weather(data, req_dt or cur_dt, seq=seq)
            src = str((payload or {}).get("source") or "API")
            sec = float(
                (payload or {}).get("elapsed")
                or getattr(self, "_weather_last_elapsed", 0.0)
                or 0.0
            )
            self._safe_show_status(f"날씨 조회 완료 · {src} · {sec:.1f}초")
        except Exception as e:
            print(f"[WorkLog] weather finished apply: {e}")
        finally:
            # 워커는 1개만 허용 → 완료 시 항상 버튼 복원(결과 적용은 seq로 가드)
            if not getattr(self, "_weather_page_closing", False):
                self._set_weather_fetch_busy(False)

    def _on_weather_fetch_failed(self, seq: int, message: str) -> None:
        try:
            if self._weather_ui_accepts_result(seq):
                msg = str(message or "").strip() or "날씨 조회에 실패했습니다."
                # 기존 화면값은 유지하고 안내만
                self._safe_show_status(msg)
                if any(
                    k in msg
                    for k in ("네트워크", "인증", "시간 초과", "타임아웃", "데이터베이스")
                ):
                    QMessageBox.warning(self, "날씨 조회", msg)
        except Exception as e:
            print(f"[WorkLog] weather failed handler: {e}")
        finally:
            if not getattr(self, "_weather_page_closing", False):
                self._set_weather_fetch_busy(False)

    def _apply_fetched_weather(self, w_data: dict, requested_date: str, seq: int | None = None) -> None:
        """조회 결과를 기상 위젯에 반영. 요청 날짜·seq가 맞지 않으면 무시."""
        if not w_data:
            return
        if seq is not None and seq != self._weather_fetch_seq:
            return
        if getattr(self, "_weather_page_closing", False):
            return
        try:
            ...
            return
        if requested_date and str(requested_date)[:10] != cur_dt:
            return
        ...
        for label, val in mapping.items():
            if label not in self.weather_widgets:
                continue
            if val is None or str(val).strip() == "":
                text = "-"
            else:
                text = str(val)
            self.weather_widgets[label].setText(text)
        ...
        if idx >= 0:
            ...
            if other_idx >= 0:
                self.combo_weather.setCurrentIndex(other_idx)

    def closeEvent(self, event):
        # 실행 중 네트워크는 timeout으로 종료. UI wait() 금지·terminate() 금지.
        self._weather_page_closing = True
        self._weather_fetch_seq += 1
        self._set_weather_fetch_busy(False)
        th = self._weather_thread
        if th is not None:
            try:
                th.quit()
            except Exception:
    ...
    def save_master_data(self):
        ...
        try:
            if self.db.save_weather_data(self.current_farm_cd, work_dt, weather_data, self.my_user_id):
                self._safe_show_status(f"☀️ [{work_dt}] 기상 정보가 저장되었습니다.")
                return True # 성공 반환
            else:
                return False
        except Exception as e:
    ...
    def load_linked_resource(self, row):
        ...
        try:
            ...
            self._safe_show_status(f"📂 작업 [{work_id}] 편집 모드로 전환되었습니다.")
    ...
    def save_work_data(self):
        ...
        try:
            if self.db.save_work_details(work_dt, self.current_farm_cd, work_data_list, self.my_user_id):
                self._safe_show_status(f"✅ [{work_dt}] 작업 저장 완료")
    ...
    def on_work_selection_changed(self):
        ...
        self._safe_show_status(f"🔍 작업 ID 선택됨: {self.selected_work_id}")
