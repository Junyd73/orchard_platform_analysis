...
class NewDeliveryDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 타이틀
        layout.addWidget(QLabel("배송 정보 입력 (모두 필수)", styleSheet=MainStyles.TXT_CARD_TITLE + " color: #C62828;"))
...
class SingleDeliveryEditDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 항목명(Label)용 표준 스타일
        lbl_style = MainStyles.TXT_LABEL_BOLD + " color: #2C3E50;"
...
class PastAddressDialog(QDialog):
    ...
    def init_ui(self):
        ...
        top.addWidget(QLabel(f"'{display_name}' 고객 배송 이력", styleSheet=MainStyles.TXT_CARD_TITLE + " color: #2D5A27;"))
        top.addStretch()
        ...
        layout.addWidget(QLabel("※ 받는 분 정보를 확인하고 체크하세요.", styleSheet=MainStyles.TXT_CAPTION))
    ...
    def insert_row_item(self, data, is_new=False):
        ...
        if is_new:
            cl.addSpacing(5)
            badge = QLabel("NEW"); badge.setStyleSheet(MainStyles.BADGE_NEW)
            cl.addWidget(badge)
...
class CustomerRegisterDialog(QDialog):
    ...
    def save_customer(self):
        ...
        """
        try:
            self.db.execute_query(sql, (cid, self.farm_cd, nm, tel, today_ops().isoformat()))
...
class SalesPage(QWidget):
    ...
    def __init__(self, db_manager, session):
        ...
        self._ensure_sales_detail_schema()
    ...
    def _ensure_sales_detail_schema(self):
        """판매상세 과수 컬럼(crop_nm) 마이그레이션."""
        try:
            rows = self.db.execute_query("PRAGMA table_info(t_sales_detail)")
            cols = {str(r[1]).strip().lower() for r in (rows or [])}
            if rows and "crop_nm" not in cols:
                self.db.execute_query("ALTER TABLE t_sales_detail ADD COLUMN crop_nm TEXT")
            master_cols = {str(r[1]).strip().lower() for r in (self.db.execute_query("PRAGMA table_info(t_sales_master)") or [])}
            if master_cols and "sales_status" not in master_cols:
                self.db.execute_query("ALTER TABLE t_sales_master ADD COLUMN sales_status TEXT DEFAULT 'CONFIRMED'")
            if master_cols and "sales_source" not in master_cols:
                self.db.execute_query("ALTER TABLE t_sales_master ADD COLUMN sales_source TEXT DEFAULT 'ORDER'")
        except Exception:
            pass

    def _get_grade_count_options(self, grade_cd: str):
        """등급(중분류) 하위 과수 옵션(code_cd, code_nm) 목록."""
        g = str(grade_cd or "").strip()
        if not g:
            return []
        try:
    ...
    def _reload_grade_count_combo(self, combo: QComboBox, grade_cd: str, selected_cd: str | None = None):
        combo.blockSignals(True)
        combo.clear()
        combo.addItem("선택", "")
        for cd, nm in self._get_grade_count_options(grade_cd):
            combo.addItem(nm, cd)
        if selected_cd:
            idx = combo.findData(str(selected_cd).strip())
            if idx >= 0:
    ...
    def init_ui(self):
        ...
        self.btn_cust_search.setFixedWidth(48)
        self.btn_cust_search.setMinimumHeight(34)
        ...
        # 콤보 텍스트 잘림(긴 계정명) 방지: 내용 기반으로 가로폭 힌트 제공
        self.sales_tp.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToContentsOnFirstShow)
        self.sales_tp.setMinimumContentsLength(12)
        ...
        self.pay_method_cd.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToContentsOnFirstShow)
        self.pay_method_cd.setMinimumContentsLength(12)
        # 1. 현금성 자산(AS01) 가져오기
        ...
        for b in [self.btn_new, self.btn_search, self.btn_delete]:
            b.setStyleSheet(MainStyles.BTN_SECONDARY)
            # OS/폰트에 따라 30px 고정 높이에서 텍스트가 잘리는 경우가 있어 최소치만 지정
        ...
        # 미수금합계: 빨간색 계열 하이라이트
        self.tot_unpaid_amt.setStyleSheet(MainStyles.TXT_SUMMARY_VALUE_EMPH + " qproperty-alignment: 'AlignRight'; color: #C62828;")
        ...
        self.item_table = QTableWidget(0, 13)
        ...
        # 컬럼이 많은 테이블이므로 고정폭 강제 대신, 필요 시 가로 스크롤 허용
        self.item_table.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        
        self.item_table.setHorizontalHeaderLabels([
            "선택", "품목", "품종", "규격", "등급", "과수", "수량", "단가",
            "가액", "배송비", "합계", "배송방식", "삭제"
        ])

        header = self.item_table.horizontalHeader()
        # 글자/콤보가 잘리지 않도록 기본은 Interactive로 두고, 중요한 컬럼은 Stretch
        header.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        header.setStretchLastSection(False)

        # 컬럼 너비(기본값): 헤더/콤보/값 텍스트 잘림 방지
        column_widths = {
            0: 78,   # 선택
            1: 110,  # 품목
            2: 150,  # 품종(Stretch도 적용)
            3: 96,   # 규격
            4: 102,  # 등급
            5: 110,  # 과수
            6: 72,   # 수량
    ...
    def create_delivery_tab(self):
        ...
        self.btn_del_row = QPushButton("🗑 선택삭제"); self.btn_del_row.setStyleSheet(MainStyles.BTN_SECONDARY)
        ...
        self.lbl_selected_status.setStyleSheet(MainStyles.TXT_STATUS_GREEN)
        ...
        # 배송/결제 관리 버튼은 동일한 공통 스타일/치수로 통일
        for b in [
            self.btn_manual_reg,
            self.btn_history,
            self.btn_select_all,
    ...
    def load_sales_data(self, sales_no):
        ...
        try:
            ...
            self.current_sales_status = str(dict(m).get("sales_status") or self.SALES_STATUS_CONFIRMED).strip()
            self.current_sales_source = str(dict(m).get("sales_source") or self.SALES_SOURCE_ORDER).strip()
            ...
            for row_data in items:
                ...
                self._reload_grade_count_combo(widgets[5], widgets[4].currentData() or "")
                crop_val = str(row_data["crop_nm"] or "").strip()
                if crop_val:
                    c_idx = widgets[5].findData(crop_val)
                    if c_idx >= 0:
                        widgets[5].setCurrentIndex(c_idx)
                    else:
                        # 레거시 데이터 호환: 과수명이 텍스트로 저장된 경우 표시명으로 매칭
                        for i in range(widgets[5].count()):
                            if str(widgets[5].itemText(i) or "").strip() == crop_val:
                                widgets[5].setCurrentIndex(i)
                                break

                # [3] 수량, 단가, 배송비 세팅 (콤마 처리)
                widgets[6].setText(str(int(row_data['qty'] or 0))) # 수량
                widgets[7].setText(f"{row_data['unit_price'] or 0:,.0f}") # 단가
                widgets[9].setText(f"{row_data['ship_fee'] or 0:,.0f}") # 배송비
                
                # 배송유형(10) 세팅
                idx_d = widgets[11].findData(str(row_data['dlvry_tp'] or "").strip())
                if idx_d >= 0: widgets[11].setCurrentIndex(idx_d)
    ...
    def delete_sales_data(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            delete_queries = [
                ...
                ("DELETE FROM t_sales_detail WHERE sales_no = ? AND farm_cd = ?", (self.current_sales_no, self.session['farm_cd'])),
                ("DELETE FROM t_sales_master WHERE sales_no = ? AND farm_cd = ?", (self.current_sales_no, self.session['farm_cd']))
            ]
    ...
    def add_item_row(self, checked=False):
        ...
        btn_select.setMinimumWidth(58)
        btn_select.setMinimumHeight(30)
        ...
        # [1, 2, 3, 4, 5, 11] 콤보박스 그룹 (품목, 품종, 규격, 등급, 과수, 배송방식)
        ...
        grade_combo.currentIndexChanged.connect(
            lambda *_args, gc=grade_combo, cc=widgets[5]: self._reload_grade_count_combo(cc, gc.currentData() or "")
        )

        crop_combo = widgets[5]
        crop_combo.addItem("선택", "")

        # [6, 7, 9] 입력 위젯 (수량, 단가, 배송비) - INPUT_RIGHT 적용
        for col in [6, 7, 9]:
            edit = QLineEdit("0")
        ...
        # [8, 10] 읽기 전용 필드 (가액, 합계) - READONLY/TOTAL 적용
        for col, style in [(8, MainStyles.INPUT_READONLY), (10, MainStyles.INPUT_TOTAL)]:
            edit = QLineEdit("0")
        ...
        # [11] 배송방식 데이터 로드 및 시그널
        ship_combo = widgets[11]
        ...
        self.item_table.setCellWidget(row, 12, btn_delete)
        widgets[12] = btn_delete
    ...
    def handle_delivery_tp_change(self, row):
        ...
        combo = self.item_table.cellWidget(row, 11)
        if not combo: return
    ...
    def update_row_amount(self, row):
        ...
        try:
            ...
            qty_w = self.item_table.cellWidget(row, 6)      # 수량
            price_w = self.item_table.cellWidget(row, 7)    # 단가
            amt_w = self.item_table.cellWidget(row, 8)      # 상품가액(a)
            ship_w = self.item_table.cellWidget(row, 9)     # 배송비(b)
            total_w = self.item_table.cellWidget(row, 10)   # 합계(a+b)
    ...
    def calculate_amounts(self):
        ...
        try:
            ...
            for i in range(self.item_table.rowCount()):
                w_amt = self.item_table.cellWidget(i, 8)
                if w_amt: total_prod_amt += float(w_amt.text().replace(',', '') or 0)
                w_ship = self.item_table.cellWidget(i, 9)
    ...
    def open_manual_registration(self):
        ...
        # 2. 품목 상세 테이블에서 목표 수량(6번 컬럼) 가져오기
        try:
            qty_widget = self.item_table.cellWidget(cur_row, 6)
            # 콤마 제거 후 숫자로 변환
    ...
    def open_tracking_popup(self, row):
        ...
        try:
            qty_widget = self.item_table.cellWidget(row, 6)
            total_target_qty = int(float(qty_widget.text().replace(',', ''))) if qty_widget else 0
        except:
            total_target_qty = 0
    ...
    def on_delivery_reg_clicked(self):
        ...
        total_qty = float(self.item_table.cellWidget(row, 6).text() or 0)
    ...
    def execute_full_save(self):
        ...
        try:
            ...
            sales_status = (
                self.current_sales_status
                if self.current_sales_status in (self.SALES_STATUS_DRAFT, self.SALES_STATUS_CONFIRMED)
                else self.SALES_STATUS_CONFIRMED
            )
            sales_source = (
                self.current_sales_source
            ...
            for r in range(self.item_table.rowCount()):
                ...
                row_item_amt = get_amt(self.item_table.cellWidget(r, 8))
                row_ship_fee = get_amt(self.item_table.cellWidget(r, 9))
                row_total_amt = get_amt(self.item_table.cellWidget(r, 10))

                # 상세 테이블 INSERT
                queries.append((sql_item, (
                    detail_no, s_no, self.farm_cd, item_cb.currentData(), 
                    self.item_table.cellWidget(r, 2).currentData(), # Variety
                    self.item_table.cellWidget(r, 5).currentData(), # 과수(16~20과 등)
                    self.item_table.cellWidget(r, 4).currentData(), # Grade
                    self.item_table.cellWidget(r, 3).currentData(), # Size
                    get_amt(self.item_table.cellWidget(r, 6)),      # Qty
                    get_amt(self.item_table.cellWidget(r, 7)),      # Price
                    row_item_amt, row_ship_fee, row_total_amt, 
                    0.0, 0.0, # paid/unpaid 상세는 0원 처리
                    self.item_table.cellWidget(r, 11).currentData() if self.item_table.cellWidget(r, 11) else 'LO010100',
                    "", # 상세 rmk는 비움
                    self.user_id
                )))
    ...
    def generate_sales_no(self, sales_date_str):
        ...
        return self.db.generate_sales_no(self.farm_cd, sales_date_str)
    ...
    def open_delivery_popup_for_row(self, row):
        ...
        qty_text = self.item_table.cellWidget(row, 6).text().replace(',', '')
    ...
    def download_excel_form(self):
        ...
        try:
            ...
            pd = importlib.import_module("pandas")  # 지연 로딩: 페이지 초기화 속도 개선
    ...
    def upload_excel_delivery(self):
        ...
        # [검증 1] 배송 방식 체크
        combo_method = self.item_table.cellWidget(cur_row, 11)
    ...
    def set_active_row(self, row):
        ...
        style_map = {
            ...
            4: MainStyles.COMBO, 5: MainStyles.COMBO, 11: MainStyles.COMBO,
            6: MainStyles.INPUT_RIGHT, 7: MainStyles.INPUT_RIGHT, 9: MainStyles.INPUT_RIGHT,
            8: MainStyles.INPUT_READONLY, 10: MainStyles.INPUT_TOTAL
        }

        for r in range(self.item_table.rowCount()):
            bg = "#FFF9C4" if r == row else "white"
            for c in range(self.item_table.columnCount()):
                # 0번(선택), 12번(삭제) 버튼 컬럼은 배경색만 변경
                if c in [0, 12]: continue 
