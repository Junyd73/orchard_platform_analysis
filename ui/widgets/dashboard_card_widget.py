...
from ui.widgets.weather.weather_temp_range_chart import WeatherTempRangeChart
...
class DashboardCard(QFrame):
    ...
    def __init__(
        ...
        *,
        ...
        self._is_cost_card = str(card_id or "").strip() == "labor"
        self._is_weather_card = str(card_id or "").strip() == "weather"
    ...
    def _build_unified_header(
        ...
        *,
        ...
        lbl_icon.setStyleSheet(MainStyles.DASH_SUBCARD_TITLE + " border:none; background:transparent;")
        ...
        if title_style:
            lbl_title.setStyleSheet(f"background: transparent; border: none; {title_style}")
        else:
            lbl_title.setStyleSheet(MainStyles.DASH_CARD_TITLE + " background: transparent; border: none;")
    ...
    def _build_ui(self, title, icon, main_value, sub_desc, status, show_detail_button):
        ...
        self._inner.setStyleSheet(
            "QFrame#DashboardCardInner { background: transparent; border: none; }"
            f" QLabel {{ {MainStyles.DASH_BODY_TEXT} }}"
        )
        ...
        if self._is_market_card:
            ...
            return
        if self._is_cost_card:
            self._build_cost_ui(title, icon, show_detail_button)
            return
        if self._is_weather_card:
            self._build_weather_ui(title, icon, show_detail_button)
            return
        ...
        self.lbl_value.setStyleSheet(MainStyles.DASH_KPI_VALUE)
        self._body_layout.addWidget(self.lbl_value)

        self.lbl_sub = QLabel(sub_desc)
        self.lbl_sub.setStyleSheet(MainStyles.DASH_LABEL + " background:transparent;")
        ...
        self.lbl_status.setStyleSheet(MainStyles.DASH_LABEL + " background:transparent;")
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        if self._enable_drag and event.button() == Qt.MouseButton.LeftButton:
            ...
            if not self._drag_started:
                self.restore_normal_state()
                if (
                    (self._is_market_card or self._is_cost_card or self._is_weather_card)
    ...
    def set_main_value(self, v):
        if self._is_market_card and self._market_decision_label is not None:
            ...
            return
        if self._is_weather_card and "temp" in self._weather_labels:
            self._weather_labels["temp"].setText(str(v))
            return
    ...
    def set_sub_desc(self, t):
        if self._is_market_card:
            return
        if self._is_weather_card and "status" in self._weather_labels:
            self._weather_labels["status"].setText(str(t or ""))
            return
    ...
    def set_status(self, t):
        ...
        if self._is_cost_card:
            return
        if self._is_weather_card:
            if "status" in self._weather_labels:
                self._weather_labels["status"].setText(str(t or ""))
            return
        self.lbl_status.setText(t)

    def _build_weather_ui(self, title: str, icon: str, show_detail_button: bool):
        self._body_layout.setSpacing(4)
        header_widget, _ = self._build_unified_header(
            title,
            icon or "🌤️",
            title_style=MainStyles.DASH_CARD_TITLE,
        )
        self._body_layout.addWidget(header_widget)
        ...
        lbl_icon.setStyleSheet(
            MainStyles.DASH_KPI_VALUE
            + " background:transparent; border:none; font-size:36px; line-height:36px; color:#F59E0B;"
        )
        lbl_icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_icon.setWordWrap(False)
        left_grid.addWidget(lbl_icon, 0, 0, alignment=Qt.AlignmentFlag.AlignCenter)

        lbl_temp = QLabel("23°")
        lbl_temp.setStyleSheet(
            MainStyles.DASH_KPI_VALUE
            + " background:transparent; border:none; font-size:30px; line-height:30px;"
        )
        lbl_temp.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_temp.setWordWrap(False)
        left_grid.addWidget(lbl_temp, 0, 1, alignment=Qt.AlignmentFlag.AlignCenter)

        lbl_status = QLabel("맑음")
        lbl_status.setStyleSheet(MainStyles.DASH_BODY_TEXT + " background:transparent; border:none;")
        lbl_status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_status.setWordWrap(False)
        lbl_status.setMaximumHeight(18)
        left_grid.addWidget(lbl_status, 1, 0, alignment=Qt.AlignmentFlag.AlignCenter)

        lbl_range = QLabel("최고 24 / 최저 8")
        lbl_range.setStyleSheet(MainStyles.DASH_LABEL + " background:transparent; border:none;")
        lbl_range.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_range.setWordWrap(False)
        lbl_range.setMaximumHeight(16)
        left_grid.addWidget(lbl_range, 1, 1, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        self._weather_chart = WeatherTempRangeChart(right_panel)
        self._weather_chart.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        self._weather_chart.setMinimumHeight(126)
        right_layout.addWidget(self._weather_chart, 1, Qt.AlignmentFlag.AlignVCenter)

        content_row.addWidget(left_panel, 4)
        content_row.addWidget(right_panel, 6)
        self._body_layout.addLayout(content_row, 1)
    ...
    @staticmethod
    def _weather_icon_for_status(status: str) -> str:
        s = str(status or "").strip()
        if "비" in s:
            return "☂"
        if "눈" in s:
            return "❄"
        if "구름" in s or "흐림" in s:
            return "☁"
        if "맑" in s or "해" in s:
            return "☀"
        return "⛅"
    ...
    @staticmethod
    def _weather_icon_color_for_status(status: str) -> str:
        s = str(status or "").strip()
        if "비" in s:
            return "#3182CE"
        if "눈" in s:
    ...
    def set_weather_summary_data(self, data: dict):
        if not self._is_weather_card:
            return
        payload = data or {}
        if "icon" in self._weather_labels:
            current_status = payload.get("current_status")
            self._weather_labels["icon"].setText(self._weather_icon_for_status(current_status))
            self._weather_labels["icon"].setStyleSheet(
                MainStyles.DASH_KPI_VALUE
                + " background:transparent; border:none; font-size:36px; line-height:36px;"
                + f" color:{self._weather_icon_color_for_status(current_status)};"
            )
        if "temp" in self._weather_labels:
            self._weather_labels["temp"].setText(str(payload.get("current_temp") or "—"))
        if "status" in self._weather_labels:
            self._weather_labels["status"].setText(str(payload.get("current_status") or "정보 없음"))
        if "range" in self._weather_labels:
            high = payload.get("today_max")
            low = payload.get("today_min")
            if high is None or low is None:
                self._weather_labels["range"].setText("최고 — / 최저 —")
            else:
                self._weather_labels["range"].setText(f"최고 {high} / 최저 {low}")
        if self._weather_chart is not None:
            self._weather_chart.set_data(payload.get("daily_ranges") or [])
    ...
    @staticmethod
    def _fmt_count_amount(count, amount) -> str:
        try:
            c = int(float(count or 0))
        except Exception:
            c = 0
        try:
            a = int(float(amount or 0))
        except Exception:
            a = 0
        return f"{c}건 / {a:,}원"

    def _build_cost_stat_block(self, key_prefix: str, title: str, title_color: str):
        box = QFrame()
        box.setStyleSheet(
        ...
        lbl_title.setStyleSheet(
            f"{MainStyles.DASH_SUBCARD_TITLE} border:none;background:transparent;color:{title_color};"
        )
        lay.addWidget(lbl_title)

        clean_title = str(title or "").replace("👤 ", "").replace("💰 ", "").strip()
        month_no = datetime.datetime.now().month
        lbl_today = QLabel(f"{month_no}월 {clean_title} : 0원")
        lbl_today.setStyleSheet(
            f"{MainStyles.DASH_SUMMARY_VALUE} border:none;background:transparent;"
        )
        lay.addWidget(lbl_today)

        lbl_unpaid = QLabel(f"총 미지급 {clean_title} : 0건 / 0원")
        lbl_unpaid.setStyleSheet(
            f"{MainStyles.DASH_BODY_EMPH} border:none;background:transparent;"
        )
        lay.addWidget(lbl_unpaid)

        self._cost_labels[f"{key_prefix}_today"] = lbl_today
        self._cost_labels[f"{key_prefix}_unpaid"] = lbl_unpaid
        return box

    def _build_cost_ui(self, title: str, icon: str, show_detail_button: bool):
        ...
        self._cost_labels["summary_line"] = QLabel(
            f"{month_no}월 총비용 : 0원   총 미지급 비용 : 0건 / 0원"
        )
        self._cost_labels["summary_line"].setStyleSheet(
            f"{MainStyles.DASH_KPI_VALUE} background:transparent;border:none;"
        )
        self._cost_labels["summary_line"].setAlignment(
            Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignVCenter
        )
        top_layout.addWidget(self._cost_labels["summary_line"])

        bottom_layout = QHBoxLayout()
        bottom_layout.setContentsMargins(0, 0, 0, 0)
        bottom_layout.setSpacing(10)
        bottom_layout.addWidget(self._build_cost_stat_block("labor", "👤 인건비", "#2B6CB0"), 1)
        bottom_layout.addWidget(self._build_cost_stat_block("expense", "💰 경비", "#C05621"), 1)

        self._body_layout.addLayout(top_layout, 2)
        self._body_layout.addLayout(bottom_layout, 3)

        self.btn_detail = None

    def set_cost_summary_data(self, data: dict):
        if not self._is_cost_card:
            return
        payload = data or {}
        if "today_total" in self._cost_labels:
            self._cost_labels["today_total"].setText(self._fmt_won(payload.get("today_total", 0)))
        month_no = datetime.datetime.now().month
        if "summary_line" in self._cost_labels:
            self._cost_labels["summary_line"].setText(
                f"{month_no}월 총비용 : {self._fmt_won(payload.get('month_total', 0))}   "
                f"총 미지급 비용 : {self._fmt_count_amount(payload.get('unpaid_total_count', 0), payload.get('unpaid_total_amount', 0))}"
            )
        if "labor_today" in self._cost_labels:
            month_no = datetime.datetime.now().month
            self._cost_labels["labor_today"].setText(
                f"{month_no}월 인건비 : {self._fmt_won(payload.get('labor_month', 0))}"
            )
        if "labor_unpaid" in self._cost_labels:
            self._cost_labels["labor_unpaid"].setText(
                f"총 미지급 인건비 : {self._fmt_count_amount(payload.get('labor_unpaid_count', 0), payload.get('labor_unpaid_amount', 0))}"
            )
        if "expense_today" in self._cost_labels:
            month_no = datetime.datetime.now().month
            self._cost_labels["expense_today"].setText(
                f"{month_no}월 경비 : {self._fmt_won(payload.get('expense_month', 0))}"
            )
        if "expense_unpaid" in self._cost_labels:
            self._cost_labels["expense_unpaid"].setText(
                f"총 미지급 경비 : {self._fmt_count_amount(payload.get('expense_unpaid_count', 0), payload.get('expense_unpaid_amount', 0))}"
            )
    ...
    def _build_market_ui(self, title: str, icon: str, show_detail_button: bool):
        ...
        self._market_decision_label.setStyleSheet(MainStyles.DASH_LABEL + " color:#B7791F; background:transparent; border:none;")
    ...
    def _set_market_decision(self, decision: str):
        ...
        self._market_decision_label.setStyleSheet(
            MainStyles.DASH_LABEL + f" color:{color}; background:transparent; border:none;"
        )
    ...
    def _build_help_button(self, tooltip_text: str):
        ...
        btn.setStyleSheet(
            ...
            f"border-radius:{MARKET_HELP_BUTTON_SIZE // 2}px;padding:0;}}"
            "QToolButton:hover{background:#E8EEF5;color:#5A6573;}"
            "QToolTip{color:#1A202C;background-color:#F8FAFC;border:1px solid #CBD5E0;padding:4px;}"
        )
        return btn
