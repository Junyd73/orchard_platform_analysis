...
_QTY_EPS = 1e-9
_SALES_CLASS_COLS = ("sales_type_cd", "sales_category_cd", "sales_route_cd")
...
class OrderShipService:
    ...
    def confirm(self, payload: ShipConfirmIn) -> dict[str, Any]:
        ...
        try:
            ...
            # 판매분류 3축 — 재고/allocation/판매/선입금보다 먼저 차단 (S2C).
            self._require_sales_class_schema(cur)
    ...
    def _confirm_in_tx(
        ...
        *,
        ...
        # Stage4: 판매 master/detail/delivery 후 · order status 전에 선입금 자동배분
        if order_no and master is not None:
            self._apply_order_prepay_if_needed(
                cur,
                farm=farm,
                order_no=order_no,
                sales_no=sales_no,
                ...
                sales_dt=sales_dt,
                user_id=user_id,
                pre_pay_amt=_as_float(master.get("pre_pay_amt")),
                pre_pay_method_cd=str(master.get("pre_pay_method_cd") or "").strip()
                or None,
            )
    ...
    def _require_sales_class_schema(self, cur: sqlite3.Cursor) -> None:
        """t_sales_master 판매분류 3축 컬럼 필수. 없으면 전체 TX 변경 0."""
        if not _table_exists(cur, "t_sales_master"):
            raise ShipError(MSG_SCHEMA_PRECONDITION, code="SCHEMA_PRECONDITION")
        for col in _SALES_CLASS_COLS:
    ...
    def _load_order_master(
        ...
        cur.execute(
            """
            SELECT order_no, custm_id, status_cd, stock_status, sales_no, pre_pay_amt
            FROM t_order_master
        ...
        if _column_exists(cur, "t_order_master", "pre_pay_method_cd"):
            ...
            mrow = cur.fetchone()
            raw_method = _row_val(mrow, "pre_pay_method_cd", 0) if mrow else None
            pre_pay_method_cd = str(raw_method or "").strip() or None
        # 주문 분류 SSOT — blank/NULL 추정 백필 금지 (S2B/S2C).
        sales_type_cd = None
        season_type_cd = None
        if _column_exists(cur, "t_order_master", "sales_type_cd"):
            ...
            st_row = cur.fetchone()
            raw_st = _row_val(st_row, "sales_type_cd", 0) if st_row else None
            sales_type_cd = str(raw_st or "").strip() or None
        if _column_exists(cur, "t_order_master", "season_type_cd"):
            cur.execute(
                """
                SELECT season_type_cd
                  FROM t_order_master
                 WHERE farm_cd = ? AND order_no = ?
                """,
                (farm, order_no),
            ...
            se_row = cur.fetchone()
            raw_se = _row_val(se_row, "season_type_cd", 0) if se_row else None
            season_type_cd = str(raw_se or "").strip() or None
        return {
            "order_no": str(_row_val(row, "order_no", 0) or ""),
            "custm_id": _row_val(row, "custm_id", 1),
            "status_cd": status,
            "pre_pay_amt": _as_float(_row_val(row, "pre_pay_amt", 5)),
            "pre_pay_method_cd": pre_pay_method_cd,
            "sales_type_cd": sales_type_cd,
            "season_type_cd": season_type_cd,
        }

    def _applied_order_prepay(
        self, cur: sqlite3.Cursor, farm: str, order_no: str
    ) -> float:
        """CONFIRMED 판매에 order_no provenance로 적용된 선입금 합 (동일 TX)."""
        cur.execute(
            """
            SELECT COALESCE(SUM(c.pay_amt), 0)
              FROM t_cash_ledger c
              JOIN t_sales_master s
                ON s.farm_cd = c.farm_cd
               AND s.sales_no = c.sales_no
             WHERE c.farm_cd = ?
               AND c.order_no = ?
               AND s.sales_status = ?
               AND s.order_no = c.order_no
            """,
            (farm, order_no, SALES_STATUS_CONFIRMED),
        )
        return _as_float(cur.fetchone()[0])

    def _get_remaining_order_prepay(
        self, cur: sqlite3.Cursor, farm: str, order_no: str, pre_pay_amt: float
    ) -> float:
        applied = self._applied_order_prepay(cur, farm, order_no)
        return max(0.0, _as_float(pre_pay_amt) - applied)

    def _apply_order_prepay_if_needed(
        self,
        cur: sqlite3.Cursor,
        *,
        farm: str,
        ...
        user_id: str,
        pre_pay_amt: float,
        pre_pay_method_cd: str | None,
    ) -> None:
        """출고 TX 내 선입금 자동배분. DIRECT 무주문은 호출하지 않음."""
        if _as_float(pre_pay_amt) <= _QTY_EPS:
            return
        remaining = self._get_remaining_order_prepay(
            cur, farm, order_no, pre_pay_amt
        )
        if remaining <= _QTY_EPS:
            return
        method = str(pre_pay_method_cd or "").strip()
        if not method:
            raise ShipValidationError(
                MSG_PREPAY_METHOD_REQUIRED_FOR_SHIPMENT,
                code="PREPAY_METHOD_REQUIRED",
            )
        apply_amt = min(remaining, max(0.0, _as_float(tot_sales)))
        if apply_amt <= _QTY_EPS:
            return
        ...
        try:
            SalesPaymentService(self.conn).add_payment_in_tx(
                cur,
                PaymentAddIn(
                    farm_cd=farm,
                    sales_no=sales_no,
                    ...
                    pay_dt=sales_dt,
                    rmk=MSG_REMARK_ORDER_PREPAY_APPLY,
                    user_id=user_id,
                    source_order_no=order_no,
                ),
            )
        except PaymentValidationError as exc:
            raise ShipValidationError(str(exc), code=exc.code) from exc
