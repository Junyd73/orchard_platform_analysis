...
class SalesQueryService:
    ...
    def _enrich_rep_details(
        ...
        if not sales_nos:
            return
        has_weight = _column_exists(cur, "t_sales_detail", "weight")
        has_crop_nm = _column_exists(cur, "t_sales_detail", "crop_nm")
        weight_expr = "d.weight" if has_weight else "0"
        crop_nm_expr = (
            "COALESCE(cr.code_nm, d.crop_nm, '')" if has_crop_nm else "''"
        )
        crop_join = (
            """
            LEFT JOIN m_common_code cr
              ON cr.farm_cd = d.farm_cd AND cr.code_cd = d.crop_nm
            """
            if has_crop_nm
            else ""
        )
        ...
        for row in cur.fetchall() or []:
            ...
            if sno and sno not in rep_by_sales:
                rep_by_sales[sno] = {
                    ...
                    "rep_crop_nm": str(_row_val(row, "crop_nm") or ""),
                }
        for it in items:
